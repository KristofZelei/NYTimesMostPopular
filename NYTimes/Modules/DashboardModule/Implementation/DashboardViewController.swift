...
class DashboardViewController : BaseViewController, DashboardViewControllerProtocol, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let urlString = configurations[indexPath.row].articleUrl, let url = URL(string: urlString) {
            navigationController?.pushViewController(DetailsViewController(withURL: url), animated: false)
        }
    }
